...
CreateLogistic <- function(midpoint,maximum,steepness){

return(function(x){
output <- maximum/(1+exp(-steepness*(x-midpoint)))
return(output)
})
...
CreateCurve <- function(a1,b1,c1,cpp) {

return(

function(x){
b2<-b1/(cpp^c1*1000^c1)
output<-a1/(1+b2*x^c1)
return(output)
}
...
CreateCurveValues <- function(a1,b1,c1,cpp,budget,stepsize) {

x<-seq(0,budget,stepsize)
...
CreateSRCurveValues <- function(a1,b1,c1,cpp,budget,stepsize) {
...
SelectMatrixRows <- function(M,index) {
...
output <- cbind(output,M[min(nrow(M),index[i]),i])
...
### Middle-out optimization routine. It guesses an equal budget split across media channels and adjusts from there.

OptimizerMO <- function (spend,minSpend,maxSpend,stepsize,Budget,Parameters,CPP,OptimizeSalesRevenue,n) {


### Determine ROI curves

if (OptimizeSalesRevenue) {

curves <- CreateSRCurveValues(Parameters[1,1],Parameters[2,1],Parameters[3,1],CPP[1,1],Budget,stepsize)

for (i in 1:length(spend)){

curvetemp <- CreateSRCurveValues(Parameters[1,i],Parameters[2,i],Parameters[3,i],CPP[1,i],Budget,stepsize)

curves <- cbind(curves,curvetemp)

rm(curvetemp)

} } else {

curves <- CreateCurveValues(Parameters[1,1],Parameters[2,1],Parameters[3,1],CPP[1,1],Budget,stepsize)

for (i in 1:length(spend)){

curvetemp <- CreateCurveValues(Parameters[1,i],Parameters[2,i],Parameters[3,i],CPP[1,i],Budget,stepsize)
...
### Determine differences in ROI

curves <- curves[,-1]

diffs <- curves - rbind(rep(0,ncol(curves)),curves[-nrow(curves),])

diffsum <- sum(diffs)


for (i in 1:length(minSpend)){

diffs[1:(minSpend[i]+1),i] = diffsum

curves[1:(minSpend[i]),i] =  0 - Budget

temp <- diffs[(maxSpend[i]+1),i]

diffs[(maxSpend[i]+1):nrow(diffs),i] = 0

diffs[(maxSpend[i]+1),i] = temp

temp <- curves[(maxSpend[i]+1),i]

curves[(maxSpend[i]+1):nrow(curves),i] = temp

}


# Maybe do this quickly?

# temp <- which.max(abs(SelectMatrixRows(diffs,spend+2)))

# spend[temp] <- spend[temp] + floor(n) - sum(spend)


for (i in 1:ncol(diffs)) {

spend[i] = max(spend[i], which.max(diffs[2:nrow(diffs),i]))

}


rm(diffsum, Parameters, CPP, minSpend)
...
temp <- which.min(SelectMatrixRows(diffs,spend+1))

spend[temp] <- spend[temp]-1
...
spend <- CheckForBetter(spend+1,curves,temp,i)-1
...
spend <- spend*stepsize

spend[temp] <- spend[temp] + (Budget - sum(spend))
...
CheckForBetter <- function(spend,curves,lastupdate, i) {


roi <- sum(SelectMatrixRows(curves,spend))

spendMatrix <- spend

for (k in 1:length(spend)) {

spendTemp = spend

spendTemp[lastupdate] = spend[lastupdate] + min(i, spend[k]-1)

spendTemp[k] = spend[k] - min(i, spend[k]-1)

roiTemp <- sum(SelectMatrixRows(curves,spendTemp))

roi <- cbind(roi,roiTemp)

spendMatrix <- cbind(spendMatrix,spendTemp)

}

temp <- which.max(colSums(roi))

spend <- spendMatrix[,temp]
...
spendTemp[k] = spend[lastupdate]

spendTemp[lastupdate] = spend[k]
...
Optimizer <- function (spend,minSpend, stepsize,Budget,Parameters,CPP,OptimizeSalesRevenue) {
...
diffs[1:(minSpend[i]+11),i] = diffsum

}

rm(curves, diffsum, Parameters, CPP, minSpend)

# diffsum <- apply(diffs[nrow(diffs):1,],2,cumsum)

# diffsum <- diffsum[nrow(diffsum):1,]


while (sum((spend)*stepsize) > Budget) {
...
spend[temp] <- spend[temp]-10

# spend <- SearchForBetter(diffsum,spend,temp,n)
