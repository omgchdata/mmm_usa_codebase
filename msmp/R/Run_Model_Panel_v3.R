...
#  hb_var <- spec$Trans_Variable[spec$Orig_Variable != "Intercept"]
  ...
  if(length(hb_var) >= 1) {
    wt <- data.frame(unique(b2[[varyby]])[!is.na(unique(b2[[varyby]]))])
    names(wt)[1] <- varyby
    wt[,1] <- as.character(wt[,1])
  for (j in 1:length(hb_var)) {
    wt_tmp <- aggregate(obj$data[[hb_var[j]]] ~ obj$data[[varyby]], data=obj$data, FUN=sum)
    names(wt_tmp) <- c(varyby, hb_var[j])
    wt_tmp[[varyby]] <- paste(varyby, wt_tmp[[varyby]], sep="")
    
    wt <- full_join(wt, wt_tmp)
    
#    tmp <- priors$Estimate[grep(hb_var[j], priors$Variables)]
    tmp <- priors[priors$Variables == hb_var[j], c("geo", "Estimate")]
    tmp <- full_join(tmp, wt)
    
    if(spec$Sign[spec$Trans_Variable == hb_var[j]] < 0 ) {
      tmp[[hb_var[j]]][tmp$Estimate > 0] <- 0
      #priors$Prior_Mean[grep(hb_var[j], priors$Variables)] = mean(tmp[tmp<0])
    } else {
      tmp[[hb_var[j]]][tmp$Estimate < 0] <- 0
      #priors$Prior_Mean[grep(hb_var[j], priors$Variables)] = mean(tmp[tmp>=0])
    }
    priors$Prior_Mean[grep(hb_var[j], priors$Variables)] <- 
      weighted.mean(tmp$Estimate, tmp[[hb_var[j]]])
...
#      abs(mean(tmp))*spec$PriorSD_Adj[spec$Trans_Variable == hb_var[j]]
  ...
  priors$Prior_Mean[priors$Variables == "total_direct_t2_E3R3P28D15"] <- 0.003
  priors$Prior_SD[priors$Variables == "total_direct_t2_E3R3P28D15"] <- 0.004
  
  priors$Prior_Mean[priors$Variables == "crossover_src_direct_t2_E2R2P21D10"] <- 0.001
  priors$Prior_SD[priors$Variables == "crossover_src_direct_t2_E2R2P21D10"] <- 0.002
  if(0) {
  #J. Liu 2018-10-10
