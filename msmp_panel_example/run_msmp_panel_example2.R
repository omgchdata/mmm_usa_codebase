...
ModelSpecFile_CS <- paste(ProjectDirectory, ProjectName, "_cs_Variables.csv", sep="")
...
x$Week <- as.Date(x$Week, format="%m/%d/%y")
...
mod_obj$spec_transform <- read_csv(ModelSpecFile_CS, col_types=cols()) %>% 
  dplyr::filter(Include == 1) # use Include to include (1)/exclude (0) variables
...
#########################
mod_obj <- Transform_panel(mod_obj, par_bypanel = T)
...
eb <- Gen_EB_Panel(mod_obj)
# tighten the priors search variable for geo2 region 
eb$Prior_SD[eb$Variables == "GeoCodegeo2:search_t"] <- 0.02
eb$Prior_SD[eb$Variables == "GeoCodegeo3:search_t"] <- 0.02
print("Run hierarchical bayesian model using the emperical priors:")
mod_obj <- Run_Model_Panel(mod_obj, priors = eb)
