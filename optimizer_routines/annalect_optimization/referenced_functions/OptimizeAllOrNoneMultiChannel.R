...
GetOptimizeAllOrNoneMultiChannel <- function() {
...
#print(paste("Number of Weeks: ", nWeeks))
	...
	output = OptimizeAllOrNoneMultiChannel(fTotalBudget, afGRPsLY, afInputs, afCoeffs, afOverlapCoeffs, afSeasonality, nWeeks, nEffFreq, nRecFreq, nPeriod, bGross)
        output=t(output)
	...
	timediff <- difftime(endtime, starttime, units="secs")
        ...
        cat(toJSON(list("OptimizeAllOrNoneMultiChannel"=as.matrix(output),
...
tryCatch(GetOptimizeAllOrNoneMultiChannel(), error=printError)
