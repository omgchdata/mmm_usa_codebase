...
GetMultiChannelConsolidatedReach <- function() {
...
#print(paste("bAverage: ", bAverage))
	...
	fTotalBudget <- as.numeric(POST$TotalBudget)
#print(paste("Total Budget: ", fTotalBudget))

        bGross <- as.logical(POST$bGross)
#print(paste("bGross: ", bGross))

	bAllOrNone <- as.logical(POST$bAllOrNone)
#print(paste("bAllOrNone: ", bAllOrNone))
...
#print(afSeasonality)
	...
	#Get the results of Channel Planning Optimization
        if (bAllOrNone)
          OptimizationOutput=OptimizeAllOrNoneMultiChannel(fTotalBudget, afGRPsLY, afInputs, afCoeffs, afOverlapCoeffs, afSeasonality, nWeeks, nEffFreq, nRecFreq, nPeriod, bGross)
        else
          OptimizationOutput=OptimizeMultiChannelReach(fTotalBudget, afGRPsLY, afInputs, afCoeffs, afOverlapCoeffs, afSeasonality, nWeeks, nEffFreq, nRecFreq, nPeriod, bGross)
  
        
	#Replace the Investment metric of the Inputs parameter with the output of the Optimization routine
        afInputs[,2] = OptimizationOutput
  
        #Finish reformatting the Optimization Output to prepare it for JSON serialization
        OptimizationOutput=t(OptimizationOutput)
        colnames(OptimizationOutput) = colnames(afGRPsLY)
  
        #Return average weekly reach for individual media and combined
        MultiChannelReachOutput=MultiChannelReach(afGRPsLY, afInputs, afCoeffs, afOverlapCoeffs, afSeasonality, nWeeks, nEffFreq, nRecFreq, nPeriod, bAverage)
        colnames(MultiChannelReachOutput) = c(colnames(afGRPsLY), "Net.Reach", "Gross.Reach")

      	MultiChannelExclusiveReachOutput=MultiChannelExclusiveReach(afGRPsLY, afInputs, afCoeffs, afOverlapCoeffs, afSeasonality, nWeeks, nEffFreq, nRecFreq, nPeriod, 
...
tryCatch(GetMultiChannelConsolidatedReach(), error=printError)
