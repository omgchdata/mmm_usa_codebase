...
GetOptimizeMultiChannelReach <- function() {
...
#print(paste("Number of Weeks: ", nWeeks))
	...
	output = OptimizeMultiChannelReach(fTotalBudget, afGRPsLY, afInputs, afCoeffs, afOverlapCoeffs, afSeasonality, nWeeks, nEffFreq, nRecFreq, nPeriod, bGross)
        output=t(output)
	...
	timediff <- difftime(endtime, starttime, units="secs")
        ...
        cat(toJSON(list("OptimizeMultiChannelReach"=as.matrix(output),
...
tryCatch(GetOptimizeMultiChannelReach(), error=printError)
